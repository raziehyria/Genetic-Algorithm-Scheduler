...
class CourseData():
    ...
    def __init__(self):
        ...
        excel_data_df = pandas.read_excel('data/Course list and attributes.xlsx', sheet_name='S&E Courses')
        ...
        for course_row in data:
            ...
            if int(num_of_sections) > 1:
                for section_no in range(0, int(num_of_sections)):
                    self._courses_objects_list.append(Course(subject, num + "_" + str(section_no), description,
    ...
    def _resolve_concurrencty_number(self, input_str, sections):
        ...
        if input_str is None or input_str == 'No': return concurrency_max
        elif input_str == 'Yes': return int(sections)

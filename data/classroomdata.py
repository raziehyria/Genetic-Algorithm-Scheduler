...
class ClassroomData:
    ...
    def __init__(self):
        ...
        excel_data_df = pandas.read_excel('data/Course list and attributes.xlsx', sheet_name='Classroom Capacities')
        json_str = excel_data_df.to_json(orient='records')
        classroom_rows = json.loads(json_str)
        self._classrooms_objects_list = []

        for each_class in classroom_rows:
            self._classrooms_objects_list.append(Classroom(each_class.get('BLDG'), each_class.get("RM"),
                                                           each_class.get("MX"), each_class.get("TYPE")))
